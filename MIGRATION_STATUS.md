...
- [x] Gap A68 resolvido: `BASE` avançou na paridade de núcleo (`CalcBaseValue/BaseLevel/getEstimatedRepairDuration`) com recálculo autoritativo de progressão (`baseValue/level`) em `/cmd` + snapshots (`setProgression`) e resumo de reparo canônico (`repair` no `/state` + delta `setRepairSummary`)
- [x] Gap A69 resolvido: lote de `pending` migrado com evidência no tracker para wrappers/classes já cobertas pelo fluxo autoritativo (`scripts/BUILDING1.as`, `scripts/BUILDING2.as`, `scripts/BUILDING3.as`, `scripts/BUILDING4.as`, `scripts/BUILDING10.as`, `scripts/BUILDING12.as`)
- [x] Gap A70 resolvido: gate funcional legado do General Store (`BUILDING12`) aplicado no client novo (indisponível com `countdownBuild` ativo ou HP < 50%), alinhando comportamento de `canFunction` do legado em `Building Info`/overlay
- [x] Gap A71 resolvido: recontagem do tracker após lote atual (`pending=364`, `in_progress=1`, `migrated=77`, `archived=29`)
- [x] Gap A72 resolvido: núcleo de `BASE` avançado com conclusão autoritativa de fortificação (`countdownFortify -> fort+1`) no ticker legado, delta canônico `setBuildingFortification`, campo `fortification` nos contratos (`/state`, `/base/load`, parser client) e regra legada de outpost em `StartRepairAllBuildings` (clamp mínimo de 25% HP antes do repair-all)
- [x] Gap A73 resolvido: lote de wrappers Flash de UI (`Button*_CLIP`, `CheckBox*`, `CREATUREBUTTON_CLIP`) fechado como `archived` no tracker com evidência de equivalentes runtime (`legacy-btn`, filtros/toggles) no client novo; contagens recalculadas (`pending=352`, `in_progress=1`, `migrated=77`, `archived=41`)
- [x] Gap A74 resolvido: `CanFortify` fechado end-to-end no `/cmd` com operações autoritativas (`StartFortifyBuilding`, `CancelFortifyBuilding`, `FinishFortifyNow`), integração no `ApiClient`/`YardScene` (ações de contexto + atalhos `F/N`) e cobertura no smoke `/cmd`
- [x] Gap A75 resolvido: lote `BSTORAGE/BTOWER/BWALL` avançado no motor de replay autoritativo com paridade fina adicional (`BTOWER`: modo de alvo `ground/air/mixed` + padrões `multi-target/pierce/splash`; `BWALL`: penalidade geométrica por interseção com paredes vivas; `BSTORAGE`: loot de destruição apenas em storage down + chip-loot calibrado por dano em storage)
- [x] Gap A76 resolvido: pipeline de assets ganhou mapeamento semântico validado por hash para `image_asset_numeric_id` (`assets/legacy-image-semantic-map.json` + `scripts/sync-yard-assets.mjs`) com suporte a alias semântico validado por `expectedSha1` para lotes sem par 1:1 no legado
- [x] Gap A77 resolvido: próximo lote `image_asset_numeric_id` avançado com migração de `images/2347.jpg` para tile semântico (`server/public/assets/yardbg/grass/2347_grass_rock_path_tile_01.jpg`) e integração no renderer (`TERRAIN_ASSET_BY_THEME.grass`); contagens atualizadas (`pending=344`, `in_progress=4`, `migrated=82`, `archived=41`)
- [x] Gap A78 resolvido: paridade visual de janelas legadas reforçada com overlays `screenshot_border1/2/3` (assets `2656/2655/2654`) integrados ao pipeline (`assets/legacy-ui-map.json`) e aplicados nos frames runtime (`src/style.css`) com composição pixel-friendly para reduzir artefatos/bordas divergentes
- [x] Gap A79 resolvido: build web endurecido contra falha `ENOTEMPTY` no Docker (`dist/assets/* 3`) via pipeline determinístico de produção (`scripts/clean-dist.mjs` com rotação de `dist`, `vite.build.copyPublicDir=false` e cópia explícita `scripts/copy-public-to-dist.mjs`); tracker recontado para (`pending=341`, `in_progress=4`, `migrated=85`, `archived=41`)
...
- [x] Revalidação incremental pós-`BASE` fortify/completion + lotes de pending: `docker compose run --rm --no-deps client` (`typecheck`, `lint`, `test` com 75 testes totais, `build`) + `docker compose run --rm --no-deps server` (`bun install --frozen-lockfile`, `bunx tsc --noEmit`) + smokes focados (`scripts/smoke-cmd-hardening.sh`, `scripts/smoke-state-snapshot.sh`, `scripts/smoke-state-stream.sh`)
- [x] Revalidação incremental pós-paridade de frame overlays + hardening de build: `docker compose run --rm --no-deps client` (`assets:sync-yard`, `typecheck`, `test` com 77 testes totais, `build`) + `docker compose run --rm --no-deps server` (`bun install --frozen-lockfile`, `bunx tsc --noEmit`)
