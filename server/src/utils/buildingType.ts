...
export type NormalizedBuildingType = {
  code: number;
  canonicalType: string;
};

export function normalizeBuildingTypeInput(
  rawInput: string
): NormalizedBuildingType | null {
  ...
  if (prefixed) {
    ...
    if (isSupportedBuildingCode(code)) {
      return { code, canonicalType: canonicalBuildingTypeFromCode(code) };
    }
    return null;
  }
...
export function canonicalBuildingTypeFromCode(code: number): string {
...
export function coerceBuildingTypeFromRecord(
  ...
  if (typeof rawType === "string") {
    const normalized = normalizeBuildingTypeInput(rawType);
...
export function isSupportedBuildingCode(code: number): boolean {
