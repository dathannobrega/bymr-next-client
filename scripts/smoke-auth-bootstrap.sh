...
USERNAME="${BYMR_SMOKE_USERNAME:-smokeauth}"
...
echo "[smoke-auth] Waiting for API at ${API_BASE_URL}"
if ! wait_for_server; then
  echo "[smoke-auth] API did not become ready in time"
  exit 1
fi

echo "[smoke-auth] Login ${EMAIL}"
LOGIN_RESP="$(login_user)"
TOKEN="$(echo "$LOGIN_RESP" | jq -r '.token')"

if [ -z "${TOKEN}" ] || [ "${TOKEN}" = "null" ]; then
  echo "[smoke-auth] Login failed; trying register ${EMAIL}"
  REGISTER_RESP="$(register_user_with_retry)"
  if ! echo "$REGISTER_RESP" | jq -e '.user.userid | type=="number"' >/dev/null 2>&1; then
    echo "[smoke-auth] Register failed"
...
if [ -z "${TOKEN}" ] || [ "${TOKEN}" = "null" ]; then
  echo "[smoke-auth] Login failed after register fallback"
  print_response "$LOGIN_RESP"
  exit 1
fi

echo "[smoke-auth] Checking canonical state bootstrap"
STATE_RESP="$(
...
assert_json "State snapshot" "$STATE_RESP" '
  ...
  (.player.userId | type == "number") and
  (.base.baseId | type == "string") and
  (.base.yardWidth == 20 and .base.yardHeight == 14)
'

echo "[smoke-auth] Checking maproom bootstrap (home cell)"
MAPROOM_RESP="$(
  curl -sS "${API_BASE_URL}/worldmapv3/initworldmap" \
    -H "Authorization: Bearer ${TOKEN}"
)"
assert_json "Maproom init" "$MAPROOM_RESP" '
  .error == 0 and
  (.celldata | type == "array") and
  (.celldata | length >= 1) and
  (.celldata[0].bid | tostring | length > 0)
'
